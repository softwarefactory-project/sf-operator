...
oc create route passthrough kubernetes-dashboard --service=kubernetes-dashboard --port=8443 --hostname=dashboards.sftests.com
...
# Print token to login into: dashboards.sftests.com
# Remember to add dashboards.sftests.com into /etc/hosts!
kubectl create token kubernetes-dashboard

# Now use the token to reach:
curl https://dashboards.sftests.com/#/workloads?namespace=sf
