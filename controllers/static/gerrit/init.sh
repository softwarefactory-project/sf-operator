...
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all auth.type "OAUTH"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all auth.trustedOpenId "^.*$"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all auth.gitBasicAuthPolicy "HTTP"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all plugin.gerrit-oauth-provider-keycloak-oauth.root-url "https://keycloak.${FQDN}"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all plugin.gerrit-oauth-provider-keycloak-oauth.realm "SF"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all plugin.gerrit-oauth-provider-keycloak-oauth.client-id "gerrit"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all plugin.gerrit-oauth-provider-keycloak-oauth.client-secret "${KEYCLOAK_GERRIT_CLIENT_SECRET}"
...
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all sendemail.enable "true"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all sendemail.from "MIXED"
git config -f ${GERRIT_SITE}/etc/gerrit.config --replace-all sendemail.smtpServer "postfix"
