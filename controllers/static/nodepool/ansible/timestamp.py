...
This plugin will prefix ansible-playbook's stdout with human-readable
timestamps. This also makes fluent bit able to timestamp events properly,
so that build logs can be queried in order.

It is based on Ansible's "default" callback output plugin.
"""

class TimestampedDisplay(display.Display):
    ...
    def display(
        ...
        ts_msg = u"{now} | {msg}".format(now=now, msg=msg)
        super(TimestampedDisplay, self).display(
            ts_msg, color, stderr, screen_only, log_only, newline
        )
...
class CallbackModule(CallbackBase):
    '''
    Simple output callback that appends a timestampe in front of
    each line of ansible's output.
    '''
    ...
    def __init__(self):
        ...
        super(CallbackModule, self).__init__(display=TimestampedDisplay())
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        if result._task.loop and 'results' in result._result:
            ...
            if self._display.verbosity < 2 and False:
    ...
    def v2_runner_on_unreachable(self, result):
        ...
        self._display.display(msg, color=C.COLOR_UNREACHABLE, stderr=False)
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        if context.CLIARGS['check'] and False:
