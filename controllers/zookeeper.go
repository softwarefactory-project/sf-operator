...
func (r *SFController) DeployZK(enabled bool) bool {
	...
	found := r.GetM("zookeeper", &dep)
	if !found && enabled {
		r.log.V(1).Info("Zookeeper deploy not found")
		r.CreateYAMLs(strings.ReplaceAll(ZK_OBJS, "{{ NS }}", r.ns))
	} else if found {
		if !enabled {
			r.log.V(1).Info("Deleting unused ZK")
			r.DeleteR(&dep)
		}
	}
	if enabled {
...
spec:
  ...
  usages:
    ...
    - client auth
  dnsNames:
  - zookeeper-0.zookeeper-headless.{{ NS }}.svc.cluster.local
  - zookeeper-0
  - zookeeper-1.zookeeper-headless.{{ NS }}.svc.cluster.local
  - zookeeper-1
  - zookeeper-2.zookeeper-headless.{{ NS }}.svc.cluster.local
  - zookeeper-2
  issuerRef:
    name: ca-issuer
    kind: Issuer
